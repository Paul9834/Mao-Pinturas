...
<html lang="es">
    ...
    <h2>${product.getNombre()}</h2>  <br>
                <h5>${product.getCodigo_Barras()}</h5>  <br>
      <p>Nombre:    <strong> ${product.getNombre()}</strong></p>
      <p>Descripcion:   <strong> ${product.getDescripcion()}</strong></p>
      <p>Marca:   <strong> ${product.getMarca()}</strong></p>
      <p>Cantidad:   <strong> ${product.getCantidad()}</strong></p>
      <p>Tope Minimo:   <strong> ${product.getTope_Min()}</strong></p>
      <p>Tope Maximo:   <strong> ${product.getTope_Max()}</strong></p>
      <p>Precio Compra:   <strong> ${product.getPrecio_Compra()}</strong></p>
      <p>Precio Venta:   <strong> ${product.getPrecio_Venta()}</strong></p>
      <p>Fecha de Registro:   <strong> ${product.getFecha_Registrado()}</strong></p>
      <p>Fecha de Vencimiento:   <strong> ${product.getFecha_Vencimiento()}</strong></p>
      <p>Estado:   <strong> ${product.getEstado()}</strong></p>
      <p>Usuario_Registra: <strong> ${product.getUsuario()}</strong></p>  
      <p>Categoria: <strong> ${product.getCategoria()}</strong></p>  
      <p>IVA: <strong> ${product.getIVA()}</strong></p>  
      <p>Tipo IVA: <strong> ${product.getTipo_IVA()}</strong></p>  
      <br>
      `,
                position: "top"
            });
